...
public class CheckPointGateClientAPI {
    public static boolean GLOBAL_GATE_STATE = false;

    public static void openAll() {
        GLOBAL_GATE_STATE = true;

        for (CheckPointGateBlockEntity gate : CheckPointGateBlockEntity.getClientGates()) {
            gate.setLogicalOpen(true);
        }

        ClientToServer.run("open_all_gates", (player) -> {
            Level level = player.level();
            List<BlockPos> gatePositions = CheckPointGateServerManager.getCheckPointGatesInLevel(level);
    ...
    public static void openAllServer(Level level) {
        ...
        NetworkHandler.sendToAll("open_all_pg_gate");
    }
    ...
    public static void closeAllServer(Level level) {
        ...
        NetworkHandler.sendToAll("close_all_pg_gate");
    }
    ...
    public static void register(){
        ClientEventHandler.registerEvent("open_all_pg_gate", (data)->{
            openAll();
        });
        ClientEventHandler.registerEvent("close_all_pg_gate", (data)->{
            closeAll();
        });
    }
}
